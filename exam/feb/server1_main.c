...
/** 
 * This function remove from a string the characters '\n' and '\r'.
 * For working correctly this function have to be string terminator '\0'.
 **/
...
/** 
 * This function read from the socket a maximum of chars characters. 
 * It's thought for working with string ended by '\r\n' sequence.
 * It works also without the string terminator '\0'.
 * It Returns 0 if the socket was closed.
 **/
int getCommand(int socket, char* string, int chars) {
	int i=-1, toRet=1;
	
	do {
		i++;
		toRet = Recv(socket, &string[i], sizeof(char), 0);
	} while( (string[i]!='\n') && (i<chars));
	string[--i]='\0';	
